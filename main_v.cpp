...
//GtkWidget *boxDialogChat2;
//GtkWidget *boxMiddle;

GtkCellArea* cellAreaDialog;
...
void *capture_video(void *data) {
	...
	while(run_video) {
		...
		if (PQresultStatus(res) == PGRES_TUPLES_OK) {
			...
			const char* byte_data = PQgetvalue(res, 0, 0);
			size_t size = PQgetlength(res, 0, 0);

			// выполнить обратное экранирование данных
			unsigned char* unescaped_data = PQunescapeBytea(reinterpret_cast<const unsigned char*>(byte_data), &size);

			// преобразовать данные в Mat image
			image = cv::imdecode(cv::Mat(1, size, CV_8U, unescaped_data), cv::IMREAD_COLOR);
		...
		// освободить память, выделенную для данных
		//PQfreemem(unescaped_data);
